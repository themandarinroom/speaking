...
class StudentPractice {
  ...
  renderVocabulary() {
    ...
    substitution.vocabulary.forEach(item => {
      ...
      const voices = document.createElement("div"); voices.className = "vocabulary-item-voices";
      const aiListen = document.createElement("button"); aiListen.type = "button"; aiListen.className = "vocabulary-listen vocabulary-ai-listen"; aiListen.textContent = "🔊"; aiListen.setAttribute("aria-label", `${t("aiVoice")}: ${item.hanzi}`); aiListen.addEventListener("click", () => { try { speakMandarin([{ hanzi: item.hanzi }], settings.speechRate || 0.8, ""); } catch { this.status.textContent = t("speechUnsupported"); } }); voices.append(aiListen);
      if (item.teacherAudioUrl) { const teacherListen = document.createElement("button"); teacherListen.type = "button"; teacherListen.className = "vocabulary-listen vocabulary-teacher-listen"; teacherListen.textContent = "👩‍🏫"; teacherListen.setAttribute("aria-label", `${t("teacherVoice")}: ${item.hanzi}`); teacherListen.addEventListener("click", async () => { try { this.modelAudio.src = item.teacherAudioUrl; this.modelAudio.currentTime = 0; await this.modelAudio.play(); this.status.textContent = t("listening"); } catch { this.status.textContent = t("playbackFailed"); } }); voices.append(teacherListen); }
      row.append(choose, voices); grid.append(row);
    });
